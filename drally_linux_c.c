...
void __VRETRACE_WAIT_FOR_START(void){
    /* Optimización Vita: Eliminado Delay artificial para quitar input-lag */
}

void __VRETRACE_WAIT_IF_INACTIVE(void){
    /* Optimización Vita: Solo procesamos los botones sin frenar el motor */
    IO_Loop();
}
...
void __VGA13_SETMODE(void){
    if(GX.ActiveMode != VGA13){
        /* Forzamos tamaño lógico en modo VGA clásico */
...
void __VESA101_SETMODE(void){
    if(GX.ActiveMode != VESA101){
        /* Forzamos tamaño lógico en modo de alta resolución original */
        SDL_RenderSetLogicalSize(GX.Renderer, 640, 480);
